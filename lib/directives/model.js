      ...
      link: function(scope, elem, attrs) {
        ...
        crCommon.watchUntil(
          ...
          function then(scope) {
            ...
            var displayProperty = attrs.property;
            console.log('displayProperty', displayProperty);

            crResource.get(scope.schema.$ref).view('all').then(
              function success(models) {
                console.log("success models", models);

                models.rows.forEach(function(row) {

                  var name = row.value[displayProperty];
                  modelsName[name] = row.value;
                  modelsId[row.id] = row.value;
                  
                  console.log('add item', row);
                  scope.items.push(row.value[displayProperty]);
                });

                // set selected
                if (scope.model.id) {
                  scope.selectedItem = modelsId[scope.model][displayProperty];
                }
              }
            );
          }
        ...
        scope.$watch('selectedItem', function(newValue, oldValue) {
          console.log('selectedItem', modelsName[newValue]);
          scope.model.id = modelsName[newValue]._id;
        });
      }
    };
  });
