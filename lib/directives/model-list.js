        ...
        function load(startkey) {
          ...
          resource.view('all', params).then(function success(result) {
            ...
            scope.rows = result.rows.map(function(row) {
              return {
                id: row.id,
                items: scope.titles.map(function(key, i) {
                  // if (row.doc[key] && row.doc[key].id_) {
                  //   // resolve reference
                  //   var item = { value: 'loading...'};
                  //   var refId = row.doc[key].id_;
                  //   var refType = schema.properties[key]['$ref'];
                  //   var res = crResources.get(refType).load(refId).then(function(doc) {
                  //     // var refKey = key.split('.')[1];
                  //     console.log("refkey");
                  //     // item.value = refKey ? doc[key] : doc;
                  //     item.value = doc;
                  //   });
                  //   return item;
                  // }
        ...
        var unwatch = scope.$watch('type', function() {
          ...
          resource.schema().then(function(s) {
            ...
            if (!scope.headers || scope.headers.length === 0) {
              scope.titles = Object.keys(schema.properties).filter(function(key) {
