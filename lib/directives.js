...
(function() {
  ...
  module.directive(NS + 'ModelRef', function($compile, cores) {
    return {
      ...
      controller: function($scope) {
        ...
        $scope.$on('submit', function(e, model, file) {
          e.stopPropagation();
          $scope.subModel = model;
          $scope.subFile = file;
          $scope.closeModal();

          // notify parent model
          $scope.$emit('ref', refId, model, file);
        });
  ...
  function ModelCtrl($scope, cores) {
    ...
    $scope.$on('ref', function(e, id, model, file) {
      console.log('id', id, model, file);
      subModels[id] = { id: id, model: model, file: file };
    });
    ...
    $scope.save = function() {
      ...
      resource.save($scope.model, $scope.files).then(
        function success(doc) {
          ...
          // for (var p in subModels) {
          //   var m = subModels[p];
          //   m.model.parentId_ = doc._id;
    ...
    $scope.submit = function() {
      $scope.$emit('submit', $scope.model, $scope.file);
    };
